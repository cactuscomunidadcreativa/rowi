 ...
 * 🧩 DECISION AFFINITY
 * ---------------------------------------------------------
 * Evalúa la afinidad en pensamiento crítico, análisis y juicio.
 * - Enfatiza precisión y consistencia.
 * - Sinergia en talentos críticos y lógicos.
...
export async function GET(req: NextRequest) {
  try {
    ...
    const bias = Math.min(uPrefs.biasFactor, 1.03); // decisiones: menor sesgo permitido
    const calibration = 0.96;
    const closeAdj = closenessMultiplier(memberCloseness);
    const W = CTX.decision;

    let composite135 =
      (W.growth * growth + W.collab * collab + W.understand * understand) *
      bias * calibration * closeAdj;

    // 🧠 Bonus por talentos de pensamiento crítico
    const logicalTalents = ["criticalThinking", "dataMining", "modeling", "problemSolving"];
    const sharedLogical = logicalTalents.filter(
      (t) => (N(myTals[t]) ?? 0) >= 108 && (N(thTals[t]) ?? 0) >= 108
    );
    if (sharedLogical.length >= 2) composite135 *= 1.03;
