...
export default function TPDashboardPage() {
  ...
  // Spanish fallbacks for the dynamic competency / outcome label lookups
  const COMP_FALLBACKS: Record<CompTKey, string> = {
  ...
  const OUTCOME_FALLBACKS: Record<OutcomeTKey, string> = {
    outcomeEffectiveness: "Efectividad",
    outcomeRelationships: "Relaciones",
    outcomeWellbeing: "Bienestar",
    outcomeQuality: "Calidad de Vida",
  };
  const compLabel = (k: CompTKey) => t(`tpDashboard.${k}`, COMP_FALLBACKS[k]);
  const outcomeLabel = (k: OutcomeTKey) => t(`tpDashboard.${k}`, OUTCOME_FALLBACKS[k]);
  ...
  const headerBlock = (
    <div>
      <Link href="/hub/admin/tp" className="inline-flex items-center gap-2 text-sm text-[var(--rowi-muted)] hover:text-purple-500 transition-colors mb-4">
        <ArrowLeft className="w-4 h-4" /> {t("tpDashboard.backToHub", "TP Hub")}
      </Link>
      <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium bg-purple-500/20 text-purple-500 mb-3">
        <Sparkles className="w-3 h-3" /> {t("tpDashboard.badgeEQDashboard", "EQ Dashboard")}
      </span>
      <h1 className="text-3xl font-bold mb-2">{t("tpDashboard.pageTitle", "Teleperformance EQ Dashboard")}</h1>
    </div>
  );
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-sm text-[var(--rowi-muted)]">{t("tpDashboard.loading", "Cargando datos del benchmark...")}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-12 border border-dashed border-gray-200 dark:border-zinc-700 text-center">
          ...
          <p className="text-[var(--rowi-muted)]">{t("tpDashboard.emptyBenchmark", "Sin datos de benchmark todavía")}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="text-[var(--rowi-muted)]">
              {t("tpDashboard.pageSubtitle", "Métricas agregadas de inteligencia emocional de {count} evaluaciones SEI en {countries} países")
                .replace("{count}", totalAssessments.toLocaleString())
                .replace("{countries}", countryCount > 0 ? String(countryCount) : t("tpDashboard.noData", "—"))}
            </p>
          </div>
          ...
          >
            ...
            <div>
              ...
              <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400">
                <Award className="w-3 h-3" /> {t("tpDashboard.levelEnterprise", "Enterprise")}
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {TP_REGIONS.length === 0 ? (
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-6 border border-dashed border-gray-200 dark:border-zinc-700 text-center">
          <p className="text-sm text-[var(--rowi-muted)]">{t("tpDashboard.emptyBenchmark", "Sin datos de benchmark todavía")}</p>
        </div>
      ) : (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {TP_REGIONS.map((region, i) => (
          ...
          >
            ...
            <div className="text-[10px] text-[var(--rowi-muted)]">{region.count.toLocaleString()} {t("tpDashboard.assessments", "evaluaciones")}</div>
          </motion.div>
        ))}
      </div>
      )}
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-zinc-900 rounded-2xl p-8 shadow-lg border border-gray-100 dark:border-zinc-800">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
            <Brain className="w-5 h-5 text-purple-500" /> {t("tpDashboard.globalEQAverage", "Promedio EQ Global")}
          </h2>
          <div className="flex items-center justify-center">
            <EQCircle score={avgEQ} max={TP_USER_IDENTITY.eqMax} min={TP_USER_IDENTITY.eqMin} avgLabel={t("tpDashboard.tpGlobalAvg", "Promedio Global TP")} />
          </div>
          <p className="text-xs text-[var(--rowi-muted)] text-center mt-4">
            {t("tpDashboard.seiScaleLabel", "Escala SEI: 65–135 | Basado en {count} evaluaciones").replace("{count}", totalAssessments.toLocaleString())}
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-zinc-900 rounded-2xl p-8 shadow-lg border border-gray-100 dark:border-zinc-800">
          <h2 className="text-xl font-bold mb-2 flex items-center gap-2">
            <Target className="w-5 h-5 text-purple-500" /> {t("tpDashboard.threePursuits", "Las 3 Búsquedas")}
          </h2>
          <p className="text-sm text-[var(--rowi-muted)] mb-6">{t("tpDashboard.threePursuitsDesc", "El modelo SEI de Six Seconds organiza 8 competencias en 3 búsquedas")}</p>
          <div className="space-y-5">
            <PursuitBar pursuit={TP_PURSUITS.know} label={t("tpDashboard.pursuitKnow", "Conócete")} />
            <PursuitBar pursuit={TP_PURSUITS.choose} label={t("tpDashboard.pursuitChoose", "Elígete")} />
            <PursuitBar pursuit={TP_PURSUITS.give} label={t("tpDashboard.pursuitGive", "Entrégate")} />
          </div>
        </motion.div>
      </div>
      ...
      <div>
        <div className="mb-6">
          <h2 className="text-2xl font-bold mb-2 flex items-center gap-2">
            <Zap className="w-6 h-6 text-purple-500" /> {t("tpDashboard.competenciesTitle", "8 Competencias SEI")}
          </h2>
          <p className="text-[var(--rowi-muted)]">{t("tpDashboard.competenciesDesc", "Promedio vs mejores puntajes en todas las evaluaciones de TP")}</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {TP_COMPETENCIES.map((comp) => (
            <CompetencyCard key={comp.key} comp={comp} name={compLabel(comp.tKey)} avgLabel={t("tpDashboard.compAvg", "Prom")} topLabel={t("tpDashboard.compTop", "Mejor")} />
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="mb-6">
          <h2 className="text-2xl font-bold mb-2 flex items-center gap-2">
            <TrendingUp className="w-6 h-6 text-purple-500" /> {t("tpDashboard.outcomesTitle", "Resultados de Vida")}
          </h2>
          <p className="text-[var(--rowi-muted)]">{t("tpDashboard.outcomesDesc", "Cómo la inteligencia emocional impacta los resultados de vida en TP")}</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {TP_OUTCOMES.map((outcome) => (
            <OutcomeCard key={outcome.key} outcome={outcome} name={outcomeLabel(outcome.tKey)} />
          ))}
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-2xl p-6 flex gap-4">
        ...
        <div>
          <h3 className="font-semibold text-purple-900 dark:text-purple-100 mb-1">{t("tpDashboard.infoTitle", "Datos TP en Vivo")}</h3>
          <p className="text-sm text-purple-700 dark:text-purple-300">
            {t("tpDashboard.infoDesc", "Este dashboard muestra datos reales agregados de {count} evaluaciones SEI de Teleperformance. Todos los datos individuales están anonimizados. Escala SEI: 65–135.").replace("{count}", totalAssessments.toLocaleString())}
          </p>
        </div>
      </motion.div>
