...
interface Props {
  ...
  submitting?: boolean;
  onComplete: (answers: Record<string, number>, demographics: PreSeiDemographics) => void;
}

const AGE_RANGES = ["18-24", "25-34", "35-44", "45-54", "55+"];

export default function PreSeiWizard({ questions, submitting, onComplete }: Props) {
  const { t } = useI18n();
  const [step, setStep] = useState(0); // 0..questions.length-1 = preguntas; length = demografía
  ...
  const currentAnswer = current ? answers[current.sei] : undefined;
  ...
  function finish() {
    onComplete(answers, demographics);
  }
        ...
        {!onDemographics && current ? (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {step === total - 1 ? t("preSei.next", "Siguiente") : t("preSei.next", "Siguiente")}
              </button>
            </div>
          </motion.div>
        ) : (
